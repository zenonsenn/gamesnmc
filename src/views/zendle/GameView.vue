...
const progressGame = (answer: string | null) => {
  ...
  if (answerCorrect) {
    ...
    if (answerExists) {
      ...
      document.getElementById('multiplier-penalty-modal')?.classList.add('animate-penalty-wiggle')
    ...
    if (answers.value!.length > 0 && lastPlayerLetter == firstInputLetter.value) {
      ...
      multiplier.value += currentDifficultyMultiplier.value / (10.0 * Math.sqrt(multiplier.value))
      ...
      if (currentDifficulty.value == 'Easy' && differenceLetter.value > 10.0) {
        ...
        finalScore.value -= differenceLetter.value
      } else if (currentDifficulty.value == 'Lexicomaxxer' && differenceLetter.value > 100.0) {
      ...
      setTimeout(() => {
        ...
        document.getElementById('penalty-modal-letter')?.classList.add('flex', 'animate-penalty')
      ...
      finalScore.value += 10.0 * letterWeights.value[firstInputLetter.value.charCodeAt(0) - 65]
    }
    ...
    // Attempt 15 times to find a word with a different second letter for variety
    ...
    try {
      if (
        localStorage.getItem(currentDifficulty.value.toLowerCase() + 'LatestScore') == null &&
        localStorage.getItem(currentDifficulty.value.toLowerCase() + 'HighestS  core') == null
      ) {
        ...
        const maxHighScore = Math.max(
          Number.parseFloat(
            localStorage.getItem(currentDifficulty.value.toLowerCase() + 'HighestScore')!,
          ),
          finalScore.value,
        )
        // Based on difficulty
  ...
  <div>
    <div class="prevent-select h-screen bg-calm-grey">
              ...
              >
                ...
                >
                  ...
                  >
                    ...
                    <button v-on:click="closeCopyModal" class="flex cursor-pointer items-baseline">
                      <span class="material-symbols-outlined"> close </span>
                    </button>
                  </div>
                </div>
      ...
      <div class="relative flex w-full items-center justify-center">
        <!-- Cookies information -->
        <div id="cookies-modal" class="absolute top-4 hidden w-full max-w-[600px] px-4 sm:p-0">
          <div class="z-10 flex flex-row items-center rounded-lg bg-gray-300 p-3">
            Your scores are stored in your browser's local storage; that's it. We don't have servers
            to collect any of your data.
      ...
      <div class="relative flex h-screen w-full flex-col">
                ...
                <div class="flex items-center justify-center">
                  <div class="flex items-center justify-center">
                    ...
                    <div class="relative">
                      ...
                      <div id="penalty-modal" class="absolute top-4 left-4 hidden text-red-500">
                  ...
                  <div class="flex items-center justify-center">
                    | x
                    <p id="multiplier-penalty-modal" class="">{{ multiplier.toFixed(2) }}&nbsp;</p>
                  </div>
                </div>
                <div class="flex items-center justify-center">
        ...
        <div class="flex h-full min-h-0 items-center justify-center px-4">
          ...
          >
            <div class="relative flex h-full min-h-0 w-full flex-col items-start justify-center">
              <div
                id="playing-area"
              ...
              >
                ...
                <div v-if="answers.length > 0" class="flex flex-row">
                  <div
                    v-if="lastPlayerAnswer.length == 0 && answers[answers.length - 1].length <= 8"
                    class="text-gray-300"
                  >
                    {{
                  ...
                  <div
                    v-else-if="
                      lastPlayerAnswer.length == 0 && answers[answers.length - 1].length > 8
                    "
                  ...
                  <div
                    v-else-if="lastPlayerAnswer.length != 0 && lastPlayerAnswer.length > 8"
                    class="flex text-gray-300"
                  >
                  ...
                  <div v-else class="text-gray-300">
                    {{ lastPlayerAnswer.substring(0, lastPlayerAnswer.length - 1) }}
                  </div>
                ...
                <div class="flex w-full flex-col gap-y-2">
                  <div v-if="targetLength != 69" class="flex flex-row flex-wrap">
                    {{ lastCorrectLetter }}
                    <div v-for="uscore in new Array(targetLength - 1).fill('_')" :key="uscore">
                      {{ uscore }}
                    </div>
                  </div>
                  ...
                  <div class="flex w-full flex-row flex-wrap">
                    <div id="terminal-prefix" class="animate-terminal font-martian-mono text-3xl">
                      _
                    </div>
                    <div class="relative flex w-fit font-martian-mono text-3xl wrap-anywhere">
                      <!-- Answer field -->
              ...
              >
                <div v-if="playerHighestScoreEver > 0">
                  Your {{ difficultyAtStart.toLowerCase() }} difficulty game is over because
                  {{ lastPlayerAnswer }} is unique to you but hasn't been documented yet :(
                  <br />
                  Your score is {{ finalScore.toFixed(2) }}. Your highest score for this difficulty
                  is {{ playerHighestScoreEver.toFixed(2) }}&nbsp;&nbsp;
                </div>
                <div v-else>
                  Your {{ difficultyAtStart.toLowerCase() }} difficulty game is over because
                  {{ lastPlayerAnswer }} is unique to you but hasn't been documented yet :(
                ...
                <button v-on:click="restart" class="material-symbols-outlined cursor-pointer">
                  replay
                </button>
              </div>
              <div
              ...
              >
                <div v-if="possibleWords.length != 0" class="flex h-full min-h-0 flex-col">
                  <div v-if="playerLife != -1" class="mb-2">
                    {{ lastPlayerAnswer }} is invalid! You have {{ playerLife }} more chance(s) left
              ...
              <div id="keyboard-toggle" class="absolute right-0 bottom-4 flex h-fit w-full">
                <button
